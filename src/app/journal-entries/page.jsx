...
const translations = {
  ar: {
    ...
    editTitle: ' ⁄œÌ· «·ﬁÌœ',
    successAdd: ' „ ≈÷«›… «·ﬁÌœ »‰Ã«Õ',
    successEdit: ' „  ⁄œÌ· «·ﬁÌœ »‰Ã«Õ',
    successDelete: ' „ Õ–› «·ﬁÌœ »‰Ã«Õ',
    confirmDelete: 'Â· √‰  „ √ﬂœ „‰ Õ–› Â–« «·ﬁÌœø',
    searchPlaceholder: '«»ÕÀ »—ﬁ„ «·ﬁÌœ √Ê «·Ê’›...'
  },
  en: {
  ...
  const fetchEntries = async (branchId) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.error || '›‘· ›Ì Ã·» «·ﬁÌÊœ');
      }
    } catch (error) {
      toast.error('Œÿ√ ›Ì «·‘»ﬂ… ⁄‰œ Ã·» «·ﬁÌÊœ');
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (data.success) {
        ...
        await logActivity({
          ...
          details: ` „ ≈‰‘«¡ ﬁÌœ ÌÊ„Ì… ÃœÌœ: ${data.entry.entry_number}`
        });
      } else {
        toast.error(data.error || '›‘· ›Ì ≈÷«›… «·ﬁÌœ');
      }
    } catch (error) {
      toast.error('Œÿ√ ›Ì «·‘»ﬂ… ⁄‰œ ≈÷«›… «·ﬁÌœ');
    }
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.error || '›‘· ›Ì  ⁄œÌ· «·ﬁÌœ');
      }
    } catch (error) {
      toast.error('Œÿ√ ›Ì «·‘»ﬂ… ⁄‰œ  ⁄œÌ· «·ﬁÌœ');
    }
  };
  ...
  const handleDeleteClick = async (entryId) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.error || '›‘· ›Ì Õ–› «·ﬁÌœ');
      }
    } catch (error) {
      toast.error('Œÿ√ ›Ì «·‘»ﬂ… ⁄‰œ Õ–› «·ﬁÌœ');
    }
  };
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 mb-6 space-y-4">
        ...
        <div className="mt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-2 mb-2">
            ...
            <input name="description" value={line.description} onChange={handleLineChange} placeholder="«·Ê’›" className="border rounded px-3 py-2" />
          ...
          <table className="min-w-full divide-y divide-gray-200 mt-2">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">«·Ê’›</th>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">≈Ã—«¡« </th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
