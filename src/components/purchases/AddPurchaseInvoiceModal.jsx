...
export default function AddPurchaseInvoiceModal({ isOpen, onClose, onInvoiceAdded, getAuthHeaders }) {
  ...
  // جلب بيانات الموردين والمنتجات عند فتح النافذة
  useEffect(() => {
    if (isOpen) {
      const fetchData = async () => {
        ...
        try {
          const [suppliersRes, productsRes] = await Promise.all([
            fetch('/api/suppliers?all=true', { headers }),
          ...
          toast.error('فشل في جلب بيانات الموردين والمنتجات.');
          console.error(error);
        }
      };
      fetchData();
  ...
  const handleAddItem = (product) => {
    ...
    const newItem = {
      ...
      name: product.name,
      quantity: 1,
      unit_price: product.purchase_price, // استخدام سعر الشراء
    ...
    setInvoiceItems(items =>
      items.map(item => {
        if (item.product_id === productId) {
          ...
          return { ...item, unit_price: newPrice, total_price: newPrice * item.quantity };
        }
        return item;
      })
    );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!selectedSupplierId) return toast.error('يرجى اختيار مورد.');
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-start pt-10 overflow-y-auto">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-4xl">
        <div className="flex justify-between items-center mb-6 border-b pb-4">
          <h2 className="text-2xl font-bold text-gray-800">فاتورة مشتريات جديدة</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-800 text-2xl">&times;</button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label htmlFor="supplier" className="block text-sm font-medium text-gray-700 text-right">المورد</label>
              <select id="supplier" value={selectedSupplierId} onChange={(e) => setSelectedSupplierId(e.target.value)} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-right">
                <option value="">-- اختر المورد --</option>
                {suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>
            <div>
              <label htmlFor="supplierInvoiceNumber" className="block text-sm font-medium text-gray-700 text-right">رقم فاتورة المورد</label>
              <input id="supplierInvoiceNumber" type="text" value={supplierInvoiceNumber} onChange={(e) => setSupplierInvoiceNumber(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-right" />
            </div>
            <div>
          ...
          <div className="relative">
            ...
            {filteredProducts.length > 0 &&
              <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded-md mt-1 max-h-60 overflow-y-auto">{filteredProducts.map(p => (<li key={p.id} onClick={() => handleAddItem(p)} className="px-3 py-2 hover:bg-gray-100 cursor-pointer text-right">{p.name}</li>))}</ul>}
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase w-32">سعر الشراء</th>
                  ...
                  invoiceItems.map(item => (
                    <tr key={item.product_id}>
                      ...
                      <td className="px-4 py-2 whitespace-nowrap">
                        <input type="number" value={item.quantity} onChange={(e) => handleQuantityChange(item.product_id, e.target.value)} min="1" className="w-20 border border-gray-300 rounded-md px-2 py-1 text-center" />
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <input type="number" value={item.unit_price} onChange={(e) => handlePriceChange(item.product_id, e.target.value)} min="0" step="0.01" className="w-28 border border-gray-300 rounded-md px-2 py-1 text-center" />
                      </td>
          ...
          <div className="flex justify-end mt-6">
            <div className="text-xl font-bold text-gray-800">الإجمالي: {Number(totalAmount).toLocaleString()} ج.م</div>
          </div>
