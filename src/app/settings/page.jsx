...
export default function SettingsPage() {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    alert(settings.language === 'ar' ? ' „ Õ›Ÿ «·≈⁄œ«œ«  »‰Ã«Õ!' : 'Settings saved successfully!');
  };
      ...
      <form onSubmit={handleSave} className="space-y-4 bg-white rounded-lg shadow p-6">
        ...
        <div>
          ...
          <select name="currency" value={settings.currency} onChange={handleChange} className="border rounded px-3 py-2 w-full">
            <option value="EGP">Ã‰ÌÂ „’—Ì (EGP)</option>
            <option value="SAR">—Ì«· ”⁄ÊœÌ (SAR)</option>
            <option value="USD">œÊ·«— √„—ÌﬂÌ (USD)</option>
            <option value="EUR">ÌÊ—Ê (EUR)</option>
          </select>
        </div>
        <div>
