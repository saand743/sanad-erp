...
export default function AddProductModal({ isOpen, onClose, onProductAdded, getAuthHeaders }) {
  ...
  useEffect(() => {
    if (isOpen) {
      // جلب جميع الأكواد للتحقق من التكرار
      const fetchBarcodes = async () => {
        try {
          const headers = getAuthHeaders();
          const res = await fetch('/api/products?fields=product_code', { headers });
          const data = await res.json();
          if (data.success && Array.isArray(data.products)) {
            setAllBarcodes(data.products.map(p => p.product_code));
          }
        } catch {}
      };
      fetchBarcodes();
  ...
  const handleChange = (e) => {
    ...
    if (name === 'barcode') {
      if (allBarcodes.includes(value)) {
        setBarcodeError('هذا الباركود مستخدم بالفعل لمنتج آخر');
      } else {
        setBarcodeError('');
      }
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4" dir="rtl">
          ...
          <div>
            <label htmlFor="barcode" className="block text-sm font-medium text-gray-700 text-right">باركود المنتج (اختياري)</label>
            <input id="barcode" name="barcode" type="text" value={formData.barcode || ''} onChange={handleChange} placeholder="يمكنك إدخال أو مسح الباركود مباشرة" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
